...
async function loadPost() {
  ...
  try {
    ...
    if (post) {
      ...
      // Conditionally show the delete and update buttons if the logged-in user is the post author
      if (post.author?.name === loggedInUserName) {
        document.getElementById('deletePostButton').style.display = 'block';
        document.getElementById('updatePostButton').style.display = 'block'; 
        ...
        document.getElementById('updatePostButton').style.display = 'none'; 
      }

      document.getElementById("updatePostButton").addEventListener("click", function() {
        const currentUrlParams = new URLSearchParams(window.location.search);
        const postIdFromUrl = currentUrlParams.get('id');  // This already has the post ID
        window.location.href = `/post/edit/?id=${postIdFromUrl}`;  // Redirect to edit page with the post ID in the URL
      });
