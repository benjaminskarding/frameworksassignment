...
function getPostId() {
  ...
  async function loadPost() {
    ...
    try {
      const post = await readPost(postId); // Fetch the post with the author and tags
  
      if (post) {
        // Display post title, author, and body
        ...
        if (post.tags && post.tags.length > 0) {
          const tags = post.tags.join(", "); // Join tags with commas
