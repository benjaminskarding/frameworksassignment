...
async function loadPostForEditing() {
    ...
    try {
        ...
        if (mediaUrl) mediaUrl.value = post.media?.url || "";
        if (mediaAlt) mediaAlt.value = post.media?.alt || "";

        const form = document.getElementById("editPostForm");
        if (form) form.setAttribute("data-post-id", postId);
    } catch (error) {
...
// Ensure that this runs only once
